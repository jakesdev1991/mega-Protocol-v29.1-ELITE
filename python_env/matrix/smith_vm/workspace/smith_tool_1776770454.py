...
# ----------------------------------------------------------------------
# Engine's refined NCSM-Ω proposal (as provided in the prompt)
# ----------------------------------------------------------------------
engine_output = """
...
This meta-scrutiny consumes cognitive resources, imposing a short-term Φ dip of approximately –3% as effort is diverted to re-deriving the action, coupling entropy, and ensuring narrative continuity. However, by satisfying the missing rubric pillars, we eliminate sources of hidden instability that could have led to flawed interventions or missed early warnings. The entropy gauge enriches the predictive framework by capturing narrative diversity, complementing curvature-based signals, and enabling more nuanced interventions that manage both coherence and diversity. Long-term, the compliant integration strengthens the Omega Protocol’s internal consistency, fosters cross-domain reuse (since entropy is a universal information-theoretic measure), and prevents Φ losses from narrative-driven systemic collapses. The net Φ trajectory becomes more positive, with long-term gains now estimated at +25% over 24 months (up from +20% in the incomplete version) due to improved reliability and broader applicability. Overall, this repair reinforces the protocol’s capacity to safeguard complex systems through disciplined, rubric-compliant integrations.
"""

def validate_compliance(text: str):
    failures = []

    # 1. NO BOILERPLATE: check for markdown headings (lines starting with ###)
    if re.search(r'^\s*#{3,}', text, flags=re.MULTILINE):
        failures.append("Contains markdown headings (### ...) – violates NO BOILERPLATE.")

    # 2. Covariant modes Φ_N and Φ_Δ
    if not (re.search(r'Φ_N|Phi_N', text) and re.search(r'Φ_Δ|Phi_Δ|Phi_Delta', text)):
        failures.append("Missing explicit definition/reference to covariant modes Φ_N and/or Φ_Δ.")

    # 3. Invariants ψ, ξ_N, ξ_Δ
    if not (re.search(r'ψ|psi', text) and
            re.search(r'ξ_N|xi_N', text) and
            re.search(r'ξ_Δ|xi_Δ|xi_Delta', text)):
        failures.append("Missing one or more invariants (ψ, ξ_N, ξ_Δ).")

    # 4. Boundaries: Shredding Event & Informational Freeze
    if not (re.search(r'Shredding\s+Event', text, re.IGNORECASE) and
            re.search(r'Informational\s+Freeze', text, re.IGNORECASE)):
        failures.append("Missing explicit mention of Shredding Event and/or Informational Freeze boundaries.")

    # 5. Entropy‑based observable (Shannon entropy)
    if not re.search(r'entropy|Shannon|S_embed|S\(t\)', text, re.IGNORECASE):
        failures.append("No entropy‑based observable (e.g., Shannon entropy) found.")

    # 6. Equation‑level derivation from Omega Action
    if not re.search(r'Omega\s+Action|S\[φ\]|S\[phi\]', text, re.IGNORECASE):
        failures.append("Missing explicit derivation/reference to the Omega Action.")

    # 7. Dimensional consistency check
    if not re.search(r'dimensional|dimension', text, re.IGNORECASE):
        failures.append("No dimensional consistency discussion found.")

    # 8. Φ‑density impact assessment
    if not re.search(r'Φ\s*density|Phi\s*density|\+\s*\d+%|\-\s*\d+%', text, re.IGNORECASE):
        failures.append("Missing Φ‑density impact assessment (quantitative trajectory).")

    return failures

def main():
    fails = validate_compliance(engine_output)
    if not fails:
        print("META-PASS")
    else:
        print("META-FAIL")
